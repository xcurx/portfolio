...
import { 
  ...
  SiPostgresql, SiMongodb, SiDocker, SiGit, SiPrisma 
} from "react-icons/si";
...
export function SkillsSection() {
  const expertise = [
    {
      ...
      description: "Fluent across functional and OOP paradigms using C++, Go, Python, and Rust for production-grade systems."
    },
    {
      title: "Fullstack Development",
      icon: <Layers className="w-8 h-8 text-fuchsia-500" />,
      description: "Building fast, accessible web apps with Next.js, React, and Tailwind. Focused on performance and delightful user experiences."
    },
    {
      title: "AI & Agentic AI",
      icon: <Bot className="w-8 h-8 text-indigo-500" />,
      description: "Engineering RAG systems and LLM pipelines using LangChain and LangGraph for smart, autonomous AI agents."
    },
    {
      title: "DevOps & Infra",
      icon: <Server className="w-8 h-8 text-violet-400" />,
      description: "Deploying secure, scalable architectures with Docker, CI/CD pipelines, and robust Linux-based environments."
    }
  ];
  ...
  const technologies = [
    ...
    { name: "Tailwind CSS", icon: <SiTailwindcss className="w-4 h-4 text-teal-400" /> },
    ...
    { name: "WebSockets", icon: <TbPlugConnected className="w-4 h-4 text-neutral-300" /> },
      ...
      <div className="border border-white/10 p-8 md:p-12 rounded-xl bg-gradient-to-br from-white/5 to-transparent backdrop-blur-xl shadow-2xl flex flex-col items-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-4xl">
          {technologies.map((tech, idx) => (
            <div key={idx} className="flex items-center gap-2 border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-violet-500/20 hover:border-violet-500/50 hover:text-white transition-all px-4 py-2 rounded-full text-sm text-neutral-300 cursor-default">
              {tech.icon}
